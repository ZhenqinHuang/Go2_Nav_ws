...
def main():
    ...
    try:
        ...
        client.sendto(control_frame(session, 2, token, ARM_REQUEST), destination)
        ack, _ = receive_ack(client)
        assert ack["sequence"] == 2 and ack["token"] == token
        assert ack["state"] == ARMING
        ...
        while time.monotonic() < deadline:
            time.sleep(0.18)
            sequence += 1
            client.sendto(control_frame(session, sequence, token), destination)
            ack, _ = receive_ack(client)
        assert ack["state"] == ARMED

        corrupted = bytearray(
            control_frame(session, sequence + 1, token, vx=0.2)
        )
        corrupted[20] ^= 0x01
        ...
        sequence += 1
        client.sendto(
            control_frame(session, sequence, token, vx=0.2), destination
        )
        ack, _ = receive_ack(client)
        assert ack["sequence"] == sequence and ack["state"] == ARMED

        timeout_ack, source = receive_ack(client, timeout=0.8)
        assert source == destination
        assert timeout_ack["state"] == LOCKED
        assert timeout_ack["fault"] == WATCHDOG
        assert timeout_ack["token"] == token

        client.sendto(
            control_frame(session, sequence + 1, token, ARM_REQUEST),
            destination,
        )
        try:
            receive_ack(client, timeout=0.15)
            raise AssertionError("revoked token unexpectedly re-armed")
        except socket.timeout:
            pass
