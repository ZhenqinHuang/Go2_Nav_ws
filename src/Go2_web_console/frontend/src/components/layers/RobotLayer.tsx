...
export class RobotLayer extends BaseLayer {
  ...
  private createRobot(): void {
    ...
    this.createHeadingMarker(robotGroup);
  ...
  private createHeadingMarker(robotGroup: THREE.Group): void {
    const marker = new THREE.Group();
    marker.name = 'go2_heading_marker';
    ...
    halo.renderOrder = 200;
    marker.add(halo);

    const ring = new THREE.Mesh(
      new THREE.RingGeometry(0.27, 0.31, 48),
      new THREE.MeshBasicMaterial({
        color: 0x67e8f9,
        transparent: true,
        opacity: 0.95,
    ...
    // ROS base_link uses +X as forward.  The arrow is authored along +X and
    // inherits the base_link quaternion, so its tip is the robot's heading.
    const arrowShape = new THREE.Shape();
    arrowShape.moveTo(0.43, 0);
    arrowShape.lineTo(-0.10, 0.23);
