...
private:
  pcl::Registration<PointT, PointT>::Ptr create_registration() {
    if(reg_method == "NDT_OMP") {
      ...
      return ndt;
#else
      RCLCPP_ERROR(get_logger(), "NDT_CUDA is not supported in this build (compile with USE_VGICP_CUDA)");
      return nullptr;
#endif
    }
...
private:
  ...
  void globalmap_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    ...
    if(use_global_localization) {
      ...
      if (result.wait_for(std::chrono::seconds(10)) != std::future_status::ready) {
        RCLCPP_ERROR(get_logger(), "Failed to call SetGlobalMap service (timeout)");
      }
    }
  }
  ...
  bool relocalize(std::shared_ptr<std_srvs::srv::Empty::Request> req, std::shared_ptr<std_srvs::srv::Empty::Response> res) {
    ...
    if (query_result_future.wait_for(std::chrono::seconds(10)) != std::future_status::ready) {
      RCLCPP_ERROR(get_logger(), "Failed to call QueryGlobalLocalization service (timeout)");
      return false;
    }
