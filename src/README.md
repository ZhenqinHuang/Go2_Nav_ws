...
- `PCD/MID360.pcd`：原始全局地图文件
- `PCD/MID360_localization_filtered.pcd`：默认重定位地图文件
...
- 首次运行前需先用 FAST-LIO2 建图，将原始地图保存为 `Go2_localization/PCD/MID360.pcd`，并生成/使用 `MID360_localization_filtered.pcd` 作为重定位地图
