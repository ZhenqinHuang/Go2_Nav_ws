...
export function MapEditor({ connection, client, onClose }: MapEditorProps) {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      toast.success('拓扑地图已保存到本机，可通过导出文件部署');
    } catch (error) {
      console.error('Failed to save topology map:', error);
      toast.warning('拓扑地图本地保存失败');
    }

    toast.info('栅格地图编辑已保留在本机，可通过导出文件部署');
  };
  ...
  const importMapBundle = async (bundle: Blob, label: string): Promise<boolean> => {
    try {
      const { occupancyGrid, topologyMap } = await importMap(bundle);
      let imported = false;
      ...
      if (!imported) {
        toast.warning('地图包中没有找到有效的 PGM/YAML 或拓扑地图');
        return false;
      }
      toast.success(`地图已载入：${label}`);
      return true;
    } catch (error) {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.name.toLowerCase().endsWith('.zip')) {
      toast.error('请选择 ZIP 格式的地图文件');
      event.target.value = '';
      return;
    }
    await importMapBundle(file, file.name);
    event.target.value = '';
  };
    ...
    <div className="MapEditor">
      <div className="EditorHeader">
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <button
            ...
            title="从 Jetson 地图库或本地 ZIP 导入地图"
          >
            📥 导入地图
          </button>
          <button
      ...
      {showImportDialog && (
        <MapImportDialog
          ...
          onChooseLocal={() => fileInputRef.current?.click()}
          onImportBundle={importMapBundle}
        />
      )}
      {showExportDialog && (
        <div style={{
