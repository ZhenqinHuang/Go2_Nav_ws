...
export class MapManager {
  ...
  public publishTopology(_connection?: RosbridgeConnection): void {
    // The production console is deliberately read-only at the rosbridge
    // boundary. Topology edits remain local/exportable until a dedicated,
    // authenticated backend API is introduced.
    this.saveTopology();
  }
  ...
  public publishOccupancyGrid(connection?: RosbridgeConnection): void {
    // Occupancy-grid edits are intentionally export-only in the safe console.
    // Direct browser publication would bypass map-server validation.
    void connection;
  }
}
