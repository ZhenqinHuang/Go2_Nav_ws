...
def load_console():
    spec = importlib.util.find_spec("go2_web_console.console_core")
    if spec is None:
        pytest.fail("console_core module is missing")
    return importlib.import_module("go2_web_console.console_core")
...
def test_authenticated_operator_can_request_stop_without_control_lease():
    module = load_console()
    clock = FakeClock()
    policy = make_policy(module, clock)
    session = login(policy)

    assert policy.authorize_stop_action(session.session_id)
    with pytest.raises(module.AuthenticationError):
        policy.authorize_stop_action("not-a-session")
